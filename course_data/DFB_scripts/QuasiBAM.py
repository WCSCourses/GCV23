...
def CommandLine_args():
    ...
    # Analysis flags
    ap.add_argument(
        "-i", dest="ins", action="store_false", help="Turn OFF insert optimisation"
    )
    ap.add_argument(
        "-p", dest="PCR", action="store_false", help="Turn OFF PCR duplicate removal"
    )
    ap.add_argument(
        "-st",
        dest="strand",
    ...
    ap.add_argument(
        ...
        dest="workflow",
        default="default",
        help="Configured workflow [default]"
    )
    ...
    for n, s in gU.fasta_parser(args.fas):
        if n in RNAMEs:
            ...
            for name, seq in args.fastas:
                if s in seq:
                    args.orfs[name].append((n, seq.index(s), len(s)))
...
def make_fasta(tabular, cd, consolidate=True, log=None, mask_gaps=False, out_file=None):
    ...
    for cons, depth in cd:
        ...
        for locus, insertions in df.loc[df.Ins > 0, "I_Desc"].iteritems():
