...
def CommandLine_args():
    ...
    ap.add_argument(
        ...
        dest="cons",
        default=20,
        help="Consensus inclusion threshold frequency. [20.0]",
    )
    ap.add_argument(
        "-d",
        dest="depth",
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="Output file stem. [input file stem (+ cons/depth if single values)]",
    )
    ...
    if args.out_file == "":
        ...
        if len(args.cd) > 1:
            args.out_file = f"{name}.fas"
        else:
            args.out_file = f"{name}.c{args.cons[0]}.d{args.depth[0]}.fas"
