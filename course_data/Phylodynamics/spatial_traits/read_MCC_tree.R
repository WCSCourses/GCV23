...
# 26 Nov 2021 - latlon, or location possible
...
# 4 march 2019
addLatLonHPD <- function(tr, regtype=3, latlonName="latlon") {
  # get the lat-lon 80% HPD (not done in original)
  if (latlonName=="latlon") {
    res1			<- gregexpr("latlon1_80\\%HPD_1=\\{[0-9eE-\\.,]+}",tr$details)
    res2			<- gregexpr("latlon2_80\\%HPD_1=\\{[0-9eE-\\.,]+}",tr$details)
  } else {
    res1			<- gregexpr("location1_80\\%HPD_1=\\{[0-9eE-\\.,]+}",tr$details)
    res2			<- gregexpr("location2_80\\%HPD_1=\\{[0-9eE-\\.,]+}",tr$details)
  }
...
read_latlon_mcc_tr <- function( trName, latlonName="latlon" ) {
	...
	latlon1	   <- apply(as.matrix(tr$details), 1, getEl, ind=2, sep=paste0(latlonName,"1="))
	latlon1	   <- as.numeric(apply(as.matrix(latlon1), 1, getEl, ind=1, sep=","))

	latlon2	   <- apply(as.matrix(tr$details), 1, getEl, ind=2, sep=paste0(latlonName,"2="))
	latlon2    <- gsub("\\]","",latlon2)
